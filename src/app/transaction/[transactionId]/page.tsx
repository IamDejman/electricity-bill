...
export default function TransactionDetailsPage() {
  ...
  useEffect(() => {
    // Find the transaction from localStorage
    const allTransactions: any[] = [];
    
    // Get all meter numbers from localStorage
    for (let i = 0; i < localStorage.length; i++) {
      const key = localStorage.key(i);
      if (key && key.startsWith('transactions_')) {
        const meterNumber = key.replace('transactions_', '');
        const transactions = JSON.parse(localStorage.getItem(key) || '[]');
        transactions.forEach((txn: any) => {
          allTransactions.push({ ...txn, meterNumber });
        });
      }
    }
    
    // Find the specific transaction
    const foundTransaction = allTransactions.find(txn => txn.id === transactionId);
    
    if (foundTransaction) {
      setTransaction(foundTransaction);
      
      // Get customer data for the meter
      const customer = getCustomerByMeterNumber(foundTransaction.meterNumber);
      if (customer) {
  ...
  const generateReceiptPDF = () => {
    ...
    doc.text(`Meter Number: ${transaction.meterNumber}`, 20, 87);
    ...
    const fileName = `IKEDC_Receipt_${transaction.meterNumber}_${dateStr}.pdf`;
    doc.save(fileName);
  ...
  const handleBackToHistory = () => {
    if (transaction) {
      router.push(`/transactions/${transaction.meterNumber}`);
    }
  };
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="flex items-center mb-3">
        ...
        >
          ...
          <span className="text-base font-medium">Back to History</span>
        </button>
      </div>

      <div className="space-y-2 mb-4">
        <h1 className="text-2xl font-bold text-gray-900 text-center">Transaction Details</h1>
        <p className="text-gray-600 text-sm text-center">Transaction ID: {transaction.id}</p>
      </div>
            ...
            <div className="space-y-4">
              <div className="bg-gray-50 rounded-lg p-4 space-y-3">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="text-sm text-gray-900">{transaction.meterNumber}</span>
                </div>
